...
function placePanel() {
  ...
  const maxH = Math.min(320, preferAbove ? spaceAbove : spaceBelow, window.innerHeight - 24)
  placement.value = preferAbove ? 'above' : 'below'
  const top = preferAbove ? r.top - 4 - maxH : r.bottom + 4
  panelStyle.value = {
    position: 'fixed',
    top: `${Math.round(Math.max(8, top))}px`,
    left: `${Math.round(left)}px`,
    width: `${Math.round(width)}px`,
    maxHeight: `${Math.round(Math.max(80, maxH))}px`,
    overflowY: 'auto',
  }
}
